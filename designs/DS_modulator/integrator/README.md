...
This block implements a fully differential switched-capacitor integrator based on a pseudo-differential Ring Amplifier. The circuit consists of sampling capacitors, integration capacitors, clock-controlled switches, and the RingAmp differential stage documented in the lower levels of hierarchy.

The integrator operates by periodically sampling an input signal and transferring the corresponding charge to the integration capacitors. Through successive charge accumulation cycles, the circuit realizes the discrete-time integration required by the delta-sigma modulator.

## 2. Role in the Delta-Sigma Modulator

The integrator is one of the fundamental building blocks of a delta-sigma modulator. Its primary function is to accumulate the quantization error generated within the feedback loop, shaping the noise spectrum and pushing a significant portion of the quantization noise outside the signal band.

In a switched-capacitor implementation, integration is performed through the controlled transfer of charge between capacitors during successive clock phases. The amount of transferred charge is proportional to the sampled input voltage, allowing the circuit to realize an accurate discrete-time integrator whose transfer function is primarily determined by capacitor ratios rather than transistor parameters.

The use of a fully differential architecture improves immunity to common-mode disturbances, reduces even-order distortion, and increases the available signal swing. Meanwhile, the RingAmp-based implementation provides the amplification required to enforce charge conservation at the integration nodes while maintaining compatibility with low-voltage operation.

The accuracy of the integrator directly impacts the performance of the delta-sigma modulator. Finite amplifier gain, incomplete settling, capacitor mismatch, and switch nonidealities can introduce integration errors that degrade noise shaping and ultimately reduce the achievable signal-to-noise-and-distortion ratio (SNDR).
...
The integrator operates in two non-overlapping clock phases. During the sampling phase, the input voltage is stored on the sampling capacitors. During the amplification phase, the stored charge is transferred to the integration capacitors through the action of the RingAmp differential stage.

The operation of the circuit can be analyzed using the principle of charge conservation. Let (Q_i) denote the total charge stored at the end of the sampling phase and (Q_f) the total charge at the end of the integration phase. Charge conservation requires

```math
Q_i = Q_f.
```

Considering the charge stored in the sampling capacitor (C_S), the compensation capacitor (C_C), and the integration capacitor (C_I), the charge balance can be expressed as
...
The charge already stored on the integration capacitor from the previous clock cycle is
...
Substituting this expression into the charge balance equation yields
...
This result shows that the output at the current clock cycle depends on both the newly sampled input and the output from the previous cycle. Therefore, the circuit accumulates charge over time, realizing the discrete-time integration required by the delta-sigma modulator.
...
where (p=C_S/C_I) represents the integrator gain. The pole at (z=1) is characteristic of an ideal discrete-time integrator and illustrates the memory behavior required for quantization noise shaping in delta-sigma modulation.

In practice, finite amplifier gain, incomplete settling, capacitor mismatch, and switch nonidealities introduce deviations from this ideal behavior. Consequently, sufficient RingAmp gain and accurate capacitor ratios are required to ensure that the implemented transfer function remains close to the ideal integrator response.
